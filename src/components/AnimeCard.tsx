    ...
    <>
      ...
      >
        ...
        <div className="absolute inset-0 flex items-center justify-center opacity-0 scale-90 transition-all duration-300 group-hover:opacity-100 group-hover:scale-100 z-20">
          ...
          >
            <PlayCircle className="w-14 h-14 text-white shadow-2xl drop-shadow-[0_0_10px_rgba(0,0,0,0.5)] fill-blue-600/90 hover:scale-110 transition-transform" />
          </button>
        </div>
        ...
        <div className="absolute top-2 left-2 flex flex-col gap-1.5 z-10">
          {rank && (
            <div className="status-badge bg-blue-600 border border-blue-500/30">
              #{rank}
            </div>
          )}
        ...
        {isModalOpen && (
          <div className="fixed inset-0 z-[120] flex items-center justify-center p-4">
             ...
             <motion.div
               ...
               className="relative bg-[#0a0a0c] border border-white/10 rounded-[32px] w-full max-w-lg shadow-[0_0_100px_rgba(37,99,235,0.15)] flex flex-col max-h-[85vh] overflow-hidden"
             >
               {/* Modal Header */}
                 ...
                 {anime.episodesList && anime.episodesList.length > 0 ? (
                   anime.episodesList.map((ep) => (
                     <Link
                       ...
                       className="group flex items-center justify-between p-4 bg-white/5 hover:bg-blue-600/20 border border-white/5 hover:border-blue-500/30 rounded-2xl transition-all"
                     >
                       <div className="flex items-center gap-4">
                         <div className="w-10 h-10 rounded-xl bg-[#0a0a0c] border border-white/5 flex items-center justify-center font-black text-gray-500 group-hover:text-blue-400 group-hover:border-blue-500/30 transition-colors">
                           {ep.number}
                         </div>
                         <div>
                           <div className="text-sm font-bold text-white group-hover:text-blue-300 transition-colors line-clamp-1">{ep.title || `${ep.number}-qism`}</div>
                           <div className="text-[10px] font-medium text-gray-500">{new Date(ep.addedAt).toLocaleDateString()}</div>
                         </div>
                       </div>
                       <div className="w-8 h-8 rounded-full bg-white/5 flex items-center justify-center text-gray-400 group-hover:bg-blue-500 group-hover:text-white transition-all transform opacity-0 group-hover:opacity-100 group-hover:scale-110">
               ...
               <div className="p-4 border-t border-white/10 bg-[#0a0a0c] flex justify-end">
                 <Link 
                   to={`/watch/${anime.id}`}
                   className="px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-xs font-black uppercase tracking-widest transition-colors flex items-center gap-2 group"
                 >
